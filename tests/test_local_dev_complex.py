...
def mock_localhost_addr(host: str, *args, **kwargs) -> list[tuple[socket.AddressFamily, socket.SocketKind, int, str, tuple[str, int]]]:
...
class TestLocalDevComplex:
    ...
    def test_local_dev_http_localhost_https_bypass_only(self) -> None:
        ...
        """
        with patch.dict(os.environ, {"COREASON_DEV_UNSAFE_MODE": "false"}): # Explicitly false
            with patch("socket.getaddrinfo", side_effect=mock_localhost_addr):
                with pytest.raises(ValidationError) as exc:
                    CoreasonIdentityConfig(
                        domain="localhost:8080",
