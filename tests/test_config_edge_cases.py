...
class TestConfigEdgeCases:
    """Test suite for edge cases in CoreasonVerifierConfig validation."""
    ...
    def test_http_timeout_zero(self) -> None:
        ...
        config = CoreasonVerifierConfig(domain="test.com", audience="aud", http_timeout=0.0)
    ...
    def test_http_timeout_negative(self) -> None:
        ...
        config = CoreasonVerifierConfig(domain="test.com", audience="aud", http_timeout=-1.0)
        assert config.http_timeout == -1.0

    def test_http_timeout_extremely_small(self) -> None:
        """Test extremely small timeout values."""
        config = CoreasonVerifierConfig(domain="test.com", audience="aud", http_timeout=1e-6)
    ...
    def test_unsafe_local_dev_env_var_parsing(self) -> None:
        ...
        with patch.dict(
            ...
            config = CoreasonVerifierConfig(domain="test.com", audience="aud")
    ...
    def test_unsafe_local_dev_disabled_implicit_http_domain(self) -> None:
        ...
        CoreasonVerifierConfig.normalize_domain handles the domain field.
        """
        # Case 1: domain="http://test.com" -> normalized to "test.com" -> issuer defaults to "https://test.com/"
        # This is safe.
        config = CoreasonVerifierConfig(domain="http://test.com", audience="aud", http_timeout=5.0)
        ...
        config2 = CoreasonVerifierConfig(domain="test.com", audience="aud", http_timeout=5.0)
    ...
    def test_salt_precedence_constructor_over_env(self) -> None:
        ...
        with patch.dict(os.environ, {"COREASON_AUTH_PII_SALT": env_salt}):
            config = CoreasonVerifierConfig(domain="test.com", audience="aud", pii_salt=SecretStr(arg_salt))
    ...
    def test_unsafe_local_dev_does_not_bypass_salt(self) -> None:
        ...
        with patch.dict(os.environ):
            ...
            with pytest.raises(ValidationError) as exc:
                CoreasonVerifierConfig(domain="test.com", audience="aud", unsafe_local_dev=True)
    ...
    def test_multiple_configs_independent_salts(self) -> None:
        ...
        c1 = CoreasonVerifierConfig(domain="d1.com", audience="a1", pii_salt=SecretStr("salt1"))
        c2 = CoreasonVerifierConfig(domain="d2.com", audience="a2", pii_salt=SecretStr("salt2"))
