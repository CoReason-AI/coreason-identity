...
def test_validate_token_header_edge_cases(manager: IdentityManager) -> None:
    ...
    # Missing token part
    # "Bearer " is technically valid prefix check, but token would be empty string.
    # If logic is `auth_header[7:]`, "Bearer " -> "".
    # Validator might fail on empty token, or we might catch it early?
    # Current implementation: `token = auth_header[7:]` -> passes "" to validator.
    # Let's ensure validator handles it or manager should?
    # Manager doesn't explicitly check len(token) > 0.
...
def test_device_login_network_failure(manager: IdentityManager) -> None:
    ...
    with patch("coreason_identity.manager.DeviceFlowClient") as MockClient:
        ...
        mock_instance.initiate_flow.side_effect = CoreasonIdentityError("Network Error")
...
def test_await_device_token_polling_failure(manager: IdentityManager) -> None:
    ...
    with patch("coreason_identity.manager.DeviceFlowClient") as MockClient:
        mock_instance = MockClient.return_value
        mock_instance.poll_token.side_effect = CoreasonIdentityError("Polling timed out")
...
def test_await_device_token_stateless_failure(manager: IdentityManager) -> None:
    ...
    with patch("coreason_identity.manager.DeviceFlowClient") as MockClient:
        mock_instance = MockClient.return_value
        mock_instance.poll_token.side_effect = CoreasonIdentityError("Access denied")
